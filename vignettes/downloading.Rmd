---
title: "Downloading data from Dryad"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Downloading data from Dryad}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
# Authenticated chunks run only when the build opts in via
# `Sys.setenv(DRYADDATA_BUILD_LIVE_VIGNETTES = "true")` AND OAuth
# credentials are available. See `?dryaddata::dryad_vignette_live`.
knitr::opts_chunk$set(
  collapse = TRUE, comment = "#>",
  eval = dryaddata::dryad_vignette_live(require_credentials = TRUE)
)
```
...
sandbox with `dryad_use_sandbox()`; revert with
`dryad_use_sandbox(FALSE)`.)

## Register an OAuth application

1. Sign in to Dryad via ORCID at
   [https://datadryad.org/account](https://datadryad.org/account).
2. Open *API applications* and create one. Dryad will issue a *client ID*
   and *client secret*.

(Organizational, publisher, and journal accounts must be requested via
`help@datadryad.org`; see Dryad's
[API accounts guide](https://github.com/datadryad/dryad-app/blob/main/documentation/apis/api_accounts.md).
...
to store and read credentials. This is the recommended path: secrets live
in the OS keychain (Keychain on macOS, Credential Manager on Windows,
Secret Service on Linux) and never appear in a plain-text file or your
command history.
...
in a script: the secret will land in your command history and any
saved `.Rhistory`. The keyring path above avoids this entirely.

```{r}
dryad_has_credentials()
```

`dryad_auth()` exchanges credentials for an access token using the
*client credentials* grant. You almost never need to call it directly;
the download functions request a token on demand and cache it in the
session until it nears expiry.
...
* **`path = NULL`** (default): the file is written to a deterministic
  path under the package cache at `dryad_cache_dir()` (see the next
  section). A second identical call returns the cached path immediately
...
* **`path = "<some file>"`**: the file is written to the location you
  ...
  project `data/` folder, a scratch volume, a `tempfile()`, etc.).
...
`tempdir()` with a persistent location such as a project `data/`
folder.
...
`dryad_download_version()` works the same way but pins to a specific
version id.

## The cache

All downloads land under `dryad_cache_dir()` (default:
...
* `dryad_cache_info()` reports the file count and size of each subtree.
* `dryad_cache_clear()` empties one or all subtrees.
...
* Pass `path = ...` to a `dryad_download_*()` call to write directly to a
  scratch volume instead of the cache.
* The 30 request/minute anonymous rate limit applies to metadata calls;
  ...
  `httr2::req_throttle()` to keep you under both limits automatically.
