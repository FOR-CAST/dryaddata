...
data repository’s [REST API (v2)](https://datadryad.org/api). It lets
you search and inspect datasets, traverse versions and files, and
download content. OAuth2 authentication and an on-disk cache for
responses and downloaded files are included.
...
`dryaddata` talks to Dryad’s production deployment
(<https://datadryad.org>) by default.
...
To experiment without touching production data, call
[`dryad_use_sandbox()`](https://for-cast.github.io/dryaddata/reference/dryad_use_sandbox.md)
to repoint at <https://sandbox.datadryad.org>;
`dryad_use_sandbox(FALSE)` reverses it.
...
so secrets live in the OS keychain (Keychain on macOS, Credential
Manager on Windows, Secret Service on Linux), never in a plain-text file
...
finds them automatically. Pass `path =` to write the file to a specific
location, or omit it to write the file to the package cache at
...
Then restart R. **Do not** use
[`Sys.setenv()`](https://rdrr.io/r/base/Sys.setenv.html) from a script:
the secret ends up in your command history. Prefer the keyring path
above.

Downloads are written under
[`dryad_cache_dir()`](https://for-cast.github.io/dryaddata/reference/dryad_cache_dir.md)
and re-used on subsequent calls. Override the cache location with the
`dryaddata.cache_dir` option or the `DRYADDATA_CACHE_DIR` environment
variable.
...
- [`vignette("dryaddata")`](https://for-cast.github.io/dryaddata/articles/dryaddata.md):
  getting started.
- [`vignette("searching", package = "dryaddata")`](https://for-cast.github.io/dryaddata/articles/searching.md):
